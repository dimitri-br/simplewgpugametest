...
// returns pixel color
float IsEdge(in texture2D tex, in sampler samp, in vec2 coords)
{
  ...
  for (int i=-1; i<2; i++) {
   for(int j=-1; j<2; j++) {
    k++;
    pix[k] = avg_intensity(get_pixel(tex, samp, coords,float(i)*dxtex,
                                          float(j)*dytex));
   }
  }
...
vec4 toon(texture2D tex, sampler samp, vec2 coords)
{
	vec2 uv = coords;
	vec4 tc = vec4(1.0, 0.0, 0.0, 1.0);
	vec3 colorOrg = texture(sampler2D(tex, samp), uv).rgb;
	vec3 vHSV =  RGBtoHSV(colorOrg.r,colorOrg.g,colorOrg.b);
	vHSV.x = nearestLevel(vHSV.x, 0);
	vHSV.y = nearestLevel(vHSV.y, 1);
	vHSV.z = nearestLevel(vHSV.z, 2);
	float edg = IsEdge(tex, samp, uv);
